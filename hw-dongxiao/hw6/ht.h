...
struct LinearProber : public Prober {
    ...
    HASH_INDEX_T next() 
    {
        ...
        // std::cout << start_ << '_' << numProbes_ << std::endl;
        numProbes_++;
        return loc;
    }
};
...
{
    for(auto i : table_)
        if(!(i==nullptr)&&!(i->deleted)) return false;
    return true;
}
...
{
    // HASH_INDEX_T loc = hash_(p.first) % CAPACITIES[mIndex_];
    if((double)length/(double)CAPACITIES[mIndex_] > rAlpha_ -1e-6) resize();
...
void HashTable<K,V,Prober,Hash,KEqual>::remove(const KeyType& key)
{
    ...
    if(item != nullptr) item->deleted = true;
}
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    for(auto i = temp.begin(); i != temp.end(); ++i)
        if((*i) != nullptr){
            if((*i)->deleted == false){
                HASH_INDEX_T loc = probe((*i)->item.first);
                table_[loc] = (*i);
                length++;
            }
            else{
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    totalProbes_++;
    // std::cout << CAPACITIES[mIndex_] << '_' << h << '_' << hash_(key) << std::endl;
