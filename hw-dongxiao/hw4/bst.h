...
protected:
    ...
    int8_t isBalancedhelper(Node<Key, Value>* n) const;
...
{
    // DONE
    if(current_ == NULL) throw std::out_of_range("end of tree");
    ...
    while(true){
        if(key < temp->getKey()){
            if(temp->getLeft()==NULL){
                temp->setLeft(new Node<Key, Value>(key, value, temp));
                return;
            }
            temp = temp->getLeft();
        }
            ...
            if(temp->getRight()==NULL){
                temp->setRight(new Node<Key, Value>(key, value, temp));
                return;
            }
            temp = temp->getRight();
        }
...
{
    // DONE
    Node<Key, Value> *temp = internalFind(key), *temp2;
    if(temp == NULL) return;
...
void BinarySearchTree<Key, Value>::clearhelper(Node<Key, Value>* n){
    if(n == NULL) return;
    // std::cout << (int)(n->getKey()) << ' ' << (n->getLeft()==NULL?0:(int)(n->getLeft()->getKey())) << ' ' << (n->getRight()==NULL?0:(int)(n->getRight()->getKey())) << std::endl;
...
template<typename Key, typename Value>
int8_t BinarySearchTree<Key, Value>::isBalancedhelper(Node<Key, Value>* n) const
{
    if(n == NULL) return 0;
    int8_t l = isBalancedhelper(n->getLeft());
    int8_t r = isBalancedhelper(n->getRight());
    if(l == -1 || r == -1) return -1;
...
bool BinarySearchTree<Key, Value>::isBalanced() const
{
    ...
    return isBalancedhelper(root_) == -1? false:true;
}
